...
def add_like(message_id):
    like = Likes.query.filter(Likes.message_id == message_id).one_or_none()
    if like:
        db.session.delete(like)
        db.session.commit()
        return redirect('/')
    else:
        like = Likes(message_id = message_id, user_id = session['curr_user'])
        db.session.add(like)
        db.session.commit()
        return redirect('/')
...
def profile():
    ...
    if form.validate_on_submit():
        if User.authenticate(user.username,form.password.data):
            ...
            db.session.add(user)
            db.session.commit()
            return redirect('/users/'+ f"{user.id}")
        else:
            flash("NO GOOD")
...
def homepage():
    ...
    if g.user:
        ...
        return render_template('home.html', messages=messages, likes = like_ids, user = user)

    else:
        return render_template('home-anon.html')

@app.route('/users/<int:user_id>/likes')
def likepage(user_id):
